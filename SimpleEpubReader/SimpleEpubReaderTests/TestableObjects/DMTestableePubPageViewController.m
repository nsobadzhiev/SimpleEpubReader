//
//  DMTestableePubPageViewController.m
//  SimpleEpubReader
//
//  Created by Nikola Sobadjiev on 4/2/14.
//  Copyright (c) 2014 Nikola Sobadjiev. All rights reserved.
//

#import "DMTestableePubPageViewController.h"

@interface DMTestableePubPageViewController ()

@end

@implementation DMTestableePubPageViewController

- (DMePubItemIterator*)iterator
{
    return itemIterator;
}

- (DMBookmarkManager*)bookmarkManager
{
    return bookmarkManager;
}

- (void)setBookmarkManager:(DMBookmarkManager *)manager
{
    bookmarkManager = manager;
}

@end
